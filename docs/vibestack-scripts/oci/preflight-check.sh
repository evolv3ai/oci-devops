#!/usr/bin/env bash
# Universal OCI CLI Preflight (Linux, macOS, Windows via Git Bash/MSYS2; WSL treated as Linux)
# - Verifies OCI CLI presence & config
# - Offers install with MSI-first on Windows (via PowerShell), install.sh on Linux/macOS, or manual via local docs
# - Supports --non-interactive / --auto for CI or scripted runs

set -euo pipefail

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'; WHITE='\033[1;37m'; GRAY='\033[0;90m'; NC='\033[0m'

HELP_DOC="${HELP_DOC:-/docs/oci-cli-install-help.md}"
NONINTERACTIVE="${NONINTERACTIVE:-false}"   # true => do preferred auto install without prompts
AUTO="${AUTO:-false}"                       # alias for NONINTERACTIVE
PREFERRED_PLATFORM="${PREFERRED_PLATFORM:-}" # optional override: windows|linux|macos

for arg in "$@"; do
  case "$arg" in
    --non-interactive|--auto) NONINTERACTIVE=true; AUTO=true;;
    --help)
      echo "Usage: $0 [--non-interactive|--auto]"
      exit 0;;
  esac
done

echo -e "${CYAN}🔍 OCI CLI Preflight Check${NC}\n=========================\n"
